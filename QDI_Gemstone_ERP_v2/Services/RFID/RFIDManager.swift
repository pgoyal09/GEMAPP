...
/// Non-blocking startup state: which response we are waiting for.
...
/// Manages an FTDI USB-Serial RFID reader using Silion framing:
///   FF LEN CMD [DATA...] CRC_H CRC_L
///
/// Observed startup handshake:
...
final class RFIDManager: NSObject, ObservableObject, RFIDService, ORSSerialPortDelegate, @unchecked Sendable {
    ...
    private static let asyncAckDataPrefix: [UInt8] = [
        0x4D, 0x6F, 0x64, 0x75, 0x6C, 0x65, 0x74, 0x65, 0x63, 0x68
    ]
    ...
    private var applyRegionConfig: (() -> Void)?
    /// Optional: set power. Called after Boot, before Async Start, only if configured.
    private var applyPowerConfig: (() -> Void)?

    // MARK: - Timing

    private let baudRate: speed_t
    private let startupResponseTimeout: TimeInterval = 2.0
    ...
    /// Tracks whether we have an active security-scoped resource access to balance stop calls.
    ...
    private func performFullStop() {
        ...
        // Balance security-scoped resource access.
        relinquishSecurityScopedAccess()
    ...
    private func relinquishSecurityScopedAccess() {
        ...
        // Note: actual stopAccessingSecurityScopedResource() would be called on the URL
        // that was used to start. In the serial port flow this is handled by ORSSerialPort
        // or the sandbox bookmark layer. The flag ensures we never call stop without start.
    }
    ...
    private func runStartupSequence() {
        ...
        guard let path = selectedPort?.path ?? findUsbSerialPort() else {
            ...
            DispatchQueue.main.async { [weak self] in
                self?.onError?("No /dev/cu.usbserial* port. Connect reader and tap Reconnect.")
            }
            scheduleReconnectAsync()
            return
        }
        ...
        queue.asyncAfter(deadline: .now() + 0.1) { [weak self] in
            guard let self, !self.startupAborted else { return }
    ...
    private func sendGetVersionAndWait() {
        ...
        scheduleStartupTimeout(step: "GetVersionInfo", fatal: true) { [weak self] in
            self?.handleStartupFailure(
                "GetVersionInfo - no response within \(self?.startupResponseTimeout ?? 2)s"
            )
        }
    }
    ...
    private func sendBootAndWait() {
        ...
        scheduleStartupTimeout(step: "BootFirmware", fatal: true) { [weak self] in
            self?.handleStartupFailure(
                "BootFirmware - no response within \(self?.startupResponseTimeout ?? 2)s"
            )
        }
    }
    ...
    private func sendAsyncStartAndWait() {
        ...
        scheduleStartupTimeout(step: "Start Async Inventory", fatal: true) { [weak self] in
            self?.handleStartupFailure(
                "Start Async Inventory - no ack within \(self?.startupResponseTimeout ?? 2)s"
            )
        }
    }
    ...
    // MARK: - Port Configuration (POSIX termios)
    ...
    private func configurePort(_ fd: Int32) {
        ...
        t.c_cflag &= ~tcflag_t(CRTSCTS)           // hardware flow control OFF
        t.c_iflag &= ~tcflag_t(IXON | IXOFF)      // software flow control OFF
    ...
    // MARK: - Reconnect (non-blocking, capped delay)
    ...
    private func parseSilionFrames(bytes: [UInt8]) {
        ...
        while idx < count {
            ...
            // Minimum frame: FF + LEN + CMD + ST0 + ST1 + CRC_H + CRC_L = 7 bytes.
            if idx + 7 > count {
            ...
            if crcRecv != crcCalc {
                ...
                if crcFailuresCount <= 5 || crcFailuresCount % 100 == 0 {
                    debugLog(
                        "[RFID] CRC mismatch cmd=0x\(String(format: "%02X", cmd)) " +
                        "status=\(String(format: "%02X", st0))\(String(format: "%02X", st1)) " +
                        "len=\(len) crcCalc=\(String(format: "%04X", crcCalc)) " +
                        "crcRecv=\(String(format: "%04X", crcRecv))"
                    )
                }
                idx += 1
                continue
            ...
            if !isTagEvent {
                debugLog(
                    "[RFID] Frame parsed cmd=0x\(String(format: "%02X", cmd)) " +
                    "status=\(String(format: "%02X", st0)) \(String(format: "%02X", st1)) " +
                    "len=\(len) crcCalc=\(String(format: "%04X", crcCalc)) " +
                    "crcRecv=\(String(format: "%04X", crcRecv))"
                )
    ...
    // MARK: - Tag ID Derivation

    /// Derive a stable canonical tag ID from payload using shared EPC canonical rules.
    ...
    static func silionCRC16(_ bytes: [UInt8]) -> UInt16 {
