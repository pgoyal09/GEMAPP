...
struct DiamondsInventoryView: View {
    ...
    private func stoneRow(_ stone: Gemstone) -> some View {
        HoverRow(isSelected: selectedStoneID == stone.persistentModelID, onTap: {
            ...
            Text(stone.color).font(AppTypography.body).foregroundStyle(AppColors.ink).lineLimit(1).frame(minWidth: TableColumn.color, maxWidth: .infinity, alignment: .leading)
            ...
            Text(stone.depthPct.map { String(format: "%.1f", $0) } ?? "—").font(AppTypography.mono).foregroundStyle(AppColors.inkMuted).frame(minWidth: TableColumn.percent, maxWidth: .infinity, alignment: .trailing)
            Text(stone.tablePct.map { String(format: "%.0f", $0) } ?? "—").font(AppTypography.mono).foregroundStyle(AppColors.inkMuted).frame(minWidth: TableColumn.percent, maxWidth: .infinity, alignment: .trailing)
            ...
            Text(rapDiscountText(rapNet: stone.rapNetPrice, sell: stone.sellPrice)).font(AppTypography.mono).foregroundStyle(rapDiscountColor(rapNet: stone.rapNetPrice, sell: stone.sellPrice)).frame(minWidth: TableColumn.percent, maxWidth: .infinity, alignment: .trailing)
